...
if strcmp(target_feature, 'LI')
    write_mrml_bilateral_parcels(atlas_folder, result_folder, r.STFC.true_suprathreshold_parcels, ['suprathreshold_parcels_n', num2str(length(r.STFC.true_suprathreshold_parcels))]);

elseif strcmp(target_feature, 'Whole')
    write_mrml_separated_parcels(atlas_folder, result_folder, r.STFC.true_suprathreshold_parcels, 'suprathreshold_parcels');

end

for c_all = 1:length(r.STFC.significant_STFCs)
    parcels_in_significant_STFC = r.STFC.significant_STFCs{c_all};
